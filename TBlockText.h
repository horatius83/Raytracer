...
namespace NUtility
{
	template <class TColorChannel>
	class TBlockText
	{
	public:
		TBlockText()
		{
			SetColor(NGraphics::TColor<TColorChannel>(0,0,0));
		};
		void SetColor(const NGraphics::TColor<TColorChannel>& tColor);
		void Print(const std::string& sString,unsigned int uiX,unsigned int uiY,
			NGraphics::TColor<TColorChannel>* pBuffer,unsigned int uiPitch);
		void Print(const std::string& sString,unsigned int uiX,unsigned int uiY,
			TArray2D< NGraphics::TColor<TColorChannel> >& tBuffer);
	private:
		void PrintCharacter(char c,unsigned int& uiX,unsigned int uiY,
			NGraphics::TColor<TColorChannel>* pBuffer,unsigned int uiPitch);
		NGraphics::TColor<TColorChannel>	m_oColor;
	};

	template <class TColorChannel>
	void TBlockText<TColorChannel>::Print(const std::string& sString,unsigned int uiX,unsigned int uiY,TArray2D<NGraphics::TColor<TColorChannel> >& tBuffer)
	{
		if(tBuffer.pGetArray())
		{
			for(unsigned int ui=0;ui<sString.size();++ui)
				PrintCharacter(sString[ui],uiX,uiY,tBuffer.pGetArray(),tBuffer.uiGetWidth());
		}
	}

	template <class TColorChannel>
	void TBlockText<TColorChannel>::Print(const std::string &sString,unsigned int uiX,unsigned int uiY,
		NGraphics::TColor<TColorChannel>* pBuffer,unsigned int uiPitch)
	{
		if(pBuffer)
		{
	...
	template <class TColorChannel>
	void TBlockText<TColorChannel>::PrintCharacter(char c, unsigned int &uiX,unsigned int uiY,
		NGraphics::TColor<TColorChannel>* pBuffer,unsigned int uiPitch)
	{
...
/*
	...
	template <class TColorChannel>
	void TBlockText<TColorChannel>::SetColor(const NGraphics::TColor<TColorChannel> &tColor)
	{
		m_oColor = tColor;
	}
}
