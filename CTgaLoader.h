...
namespace NGraphics
{
	...
	private:
		TGA_ENCODING eReadHeader(const NUtility::TArray<unsigned char>& oData,STgaHeader& stHeader); 
		bool bCanLoadRawIndexed(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader);
		bool bCanLoadRawRgb(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader);
		bool bCanLoadIndexedRleData(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader);

		NUtility::TArray2D< NGraphics::TColor<unsigned char> >	m_oData;
	};

	TGA_ENCODING CTgaLoader::eReadHeader(const NUtility::TArray<unsigned char> &oData,STgaHeader& stHeader)
	{
		if(oData.uiGetLength())
		{
			memcpy(&stHeader,&oData[0],sizeof(STgaHeader));
	...
	bool CTgaLoader::bCanLoadRawIndexed(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader)
	{
		using namespace NUtility;
		using namespace NGraphics;
		//Allocate Palette
		TArray< TColor<unsigned char> > oPalette(stHeader.m_usColorMapLength);
	...
	bool CTgaLoader::bCanLoadRawRgb(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader)
	{
	...
	bool CTgaLoader::bCanLoadIndexedRleData(const NUtility::TArray<unsigned char>& oData,const STgaHeader& stHeader)
	{
	}

	bool CTgaLoader::bCanLoadImage(const std::wstring &sFilename)
	{
		using namespace std;	
		using namespace NUtility;
		ifstream inData;
		...
		if(inData.good())
		{
			...
			TArray<unsigned char>	oTempData(uiFileSize);
