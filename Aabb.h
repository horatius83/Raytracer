...
namespace Math
{
	...
	public:
		...
		*/
		...
		bool bIntersects(const Vector& oOrigin,const Vector& oDirection,RAY_ORIENTATION eOrientation);
		
		bool bIntersects(const Vector& oOrigin,const Vector& oDirection);
		void DoesIntersect(Vector128& oAnswer,const PacketVector& oOrigin,const PacketVector& oDirection);
		...
		inline Vector& oGetLeftPoint(){return m_oLeftPoint;};
		inline Vector& oGetRightPoint(){return m_oRightPoint;};
	...
	private:
		void DoesIntersectIndividual(Vector128& oAnswer,const PacketVector& oOrigin,
			const PacketVector& oDirection,unsigned int uiMask[]);
		Vector	m_oLeftPoint;
	...
	/*
	template <class T>
	inline void Aabb<T>::DoesIntersect(Math::Vector128 &oAnswer, const Math::PacketVector &oOrigin, 
		const Math::PacketVector &oDirection)
	{
		PacketVector oInverse;
		...
		Math::Vector128 y0,yMask,z0,zMask,sseNear,sseFar;
		//find largest min
	...
	*/
	template <class T>
	inline bool Aabb<T>::bIntersects(const Math::Vector& oOrigin,const Math::Vector& oDirection)
	{
	...
	template <class T>
	inline bool Aabb<T>::bIntersects(const Vector& oOrigin,const Vector& oDirection,RAY_ORIENTATION eOrientation)
	{
		switch(eOrientation)
		{
		case MMM:
...
/*
	...
	{
		Vector oInverse;
		oInverse.SetX(oDirection.fGetX() ? 1.0f/oDirection.fGetX() : Math::g_fMaxRay);
		oInverse.SetY(oDirection.fGetY() ? 1.0f/oDirection.fGetY() : Math::g_fMaxRay);
		oInverse.SetZ(oDirection.fGetZ() ? 1.0f/oDirection.fGetZ() : Math::g_fMaxRay);
