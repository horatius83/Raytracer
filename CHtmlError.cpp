...
#ifdef WIN32
#include "CWindowsSystemSettings.h"
#endif

namespace NUtility
{
	wofstream	CHtmlError::sm_oOutData;
	long		CHtmlError::sm_lBytes=0;

	void CHtmlError::OutputSystemSettings()
	{
		...
		sm_oOutData<<L"<table>\n";
#ifdef WIN32
		CWindowsSystemSettings oSettings;
#endif
	...
	void CHtmlError::TimeOut(tm* pTime)
	{
	...
	bool CHtmlError::bOpen(const wstring& sFileName,const wstring& sCssFilename,bool bEmbed)
	{
		if(!sm_oOutData.is_open())
		{
			...
			err(L"CHtmlError::Open() : Already open",EL_WARNING);
		return false;
	}

	void CHtmlError::Close()
	{
