...
namespace Utility
{
	...
	void HtmlError::OutputSystemSettings()
	{
		...
		s3dNow = oSettings.Has3dNow() ? L"supported" : L"not supported";
		sMmx = oSettings.HasMmx() ? L"supported" : L"not supported";
		sm_oOutData << L"<table>\n";
		sm_oOutData << L"<tr>\n\t<th>Processor</th><th>Cores</th><th>3DNow!</th><th>MMX</th>" <<
			L"<th>SSE Version</th></tr>\n";
		sm_oOutData << L"<tr>\n\t<td>" << sProcessor << "</td><td>" << oSettings.GetNumProcessors() <<
			L"</td><td>" << s3dNow << L"</td><td>" << sMmx << L"</td><td>" << oSettings.SseLevel() << L"</td>\n</tr>\n";
		sm_oOutData << L"</table>\n";
	}
	...
	bool HtmlError::Open(const wstring& sFileName, const wstring& sCssFilename, bool bEmbed)
	{
