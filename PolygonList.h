...
namespace RayTracer
{
	...
	{
	public:
		bool bCanInitialize(AlignedArray<unsigned int>& oIndices,AlignedArray<Math::Vector>& oVertices,
		...
		void GetIntersection(Math::Vector128& oResults,unsigned int uiIndex,
			Math::PacketVector& oOrigin,Math::PacketVector& oDirection);
		float fGetIntersection(unsigned int uiIndex,const Math::Vector& oOrigin,const Math::Vector& oDirection);
	
		//The index for the polygon plus the vertex index (can be 0,1, or 2)
		Math::Vector&		oGetVertex(unsigned int uiPolygonIndex,unsigned int uiVertexIndex);
		Math::Vector&		oGetNormal(unsigned int uiPolygonIndex,unsigned int uiVertexIndex);
		Math::Vector2D&	oGetTexCoords(unsigned int uiPolygonIndex,unsigned int uiVertexIndex);
	...
	inline Math::Vector& CPolygonList::oGetVertex(unsigned int uiPolygonIndex,unsigned int uiVertexIndex)
	{
		assert(uiVertexIndex<3);
		return m_oVertices[m_oIndices[uiPolygonIndex]+uiVertexIndex];
	}

	inline Math::Vector& CPolygonList::oGetNormal(unsigned int uiPolygonIndex,unsigned int uiVertexIndex)
	{
		assert(uiVertexIndex<3);
		return m_oNormals[m_oIndices[uiPolygonIndex]+uiVertexIndex];
	}

	inline Math::Vector2D& CPolygonList::oGetTexCoords(unsigned int uiPolygonIndex,unsigned int uiVertexIndex)
	{
		assert(uiVertexIndex<3);
		...
		Math::PacketVector& oOrigin,Math::PacketVector& oDirection)
	{
		...
		{
			oResults.m_fData[0] = fGetIntersection(uiIndex,Vector(oOrigin.m_fX[0],oOrigin.m_fY[0],oOrigin.m_fZ[0]),
				Vector(oDirection.m_fX[0],oDirection.m_fY[0],oDirection.m_fZ[0]));
			oResults.m_fData[1] = fGetIntersection(uiIndex,Vector(oOrigin.m_fX[1],oOrigin.m_fY[1],oOrigin.m_fZ[1]),
				Vector(oDirection.m_fX[1],oDirection.m_fY[1],oDirection.m_fZ[1]));
			oResults.m_fData[2] = fGetIntersection(uiIndex,Vector(oOrigin.m_fX[2],oOrigin.m_fY[2],oOrigin.m_fZ[2]),
				Vector(oDirection.m_fX[2],oDirection.m_fY[2],oDirection.m_fZ[2]));
			oResults.m_fData[3] = fGetIntersection(uiIndex,Vector(oOrigin.m_fX[3],oOrigin.m_fY[3],oOrigin.m_fZ[3]),
				Vector(oDirection.m_fX[3],oDirection.m_fY[3],oDirection.m_fZ[3]));
		}
	}

	inline float CPolygonList::fGetIntersection(unsigned int uiIndex,const Math::Vector &oOrigin, const Math::Vector &oDirection)
	{
		using Math::Vector;
		using Math::Vector2D;
	...
	bool CPolygonList::bCanInitialize(AlignedArray<unsigned int>& oIndices,AlignedArray<Math::Vector>& oVertices,
		AlignedArray<Math::Vector>& oNormals,AlignedArray<Math::Vector2D>& oTexCoords)
	{
		using Math::Vector;
